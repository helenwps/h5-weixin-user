	 ...
	 mounted(){
	 	...
	 	this.pageObj.specCombId=this.$route.query.specCombId;
	  	if(this.pageObj.goodsId&&this.pageObj.city){
	  		this.getMatchLawyerList(this.pageObj.goodsId,1,20);
	  	}else{
	  		this.getLawyerList(this.pageObj.searchType,this.pageObj.orderType);
	  	}
    },
    methods: {
    	/*获取匹配律师列表*/
    	getMatchLawyerList(goodsId,page,limit){
    		var that=this;
			var data={
				"specCombId":"fffea48039de44a4a1999914b8fe87d8",
    	...
    	/* 获取律师列表*/
    	getLawyerList(searchType,orderType,cityName,page,limit){
    		var that=this;
			var data={
    	...
    	comment(e){
    		...
    		if(this.pageObj.commentFlag){
    			...
    			this.data.sort(function(a,b){
    				return b.goodCommentRate-a.goodCommentRate;
    			})
    			this.data.forEach(function(item){
					console.log(item.goodCommentRate);
				})
    			...
    			this.data.sort(function(a,b){
    				return a.goodCommentRate-b.goodCommentRate;
    			})
    			this.data.forEach(function(item){
					console.log(item.goodCommentRate);
				})
    	...
    	changecaseId(name){
    		...
    		this.getLawyerList(this.pageObj.searchType,this.pageObj.orderType,name);
    	},
    	onPullingUp() {
			this.pageObj.page++;
    		var that=this;
    		setTimeout(function(){
    			if(that.pageObj.goodsId&&that.pageObj.city){
	    			that.getMatchLawyerList(that.pageObj.goodsId,city,that.pageObj.page,that.pageObj.limit);
	    		}else{
	    			that.getLawyerList(that.pageObj.searchType,that.pageObj.orderType,that.pageObj.cityName,that.pageObj.page,that.pageObj.limit);		        			
	    		}
    		},500);
		},
		rebuildScroll() {
